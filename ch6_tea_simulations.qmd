...
```{r}
## there is condition and rating... they are pawbly n length?
```
...
```{r}
## high n dist is narrower & more centered around the pop mean
## low n is more spread out & not as centered
```
...
```{r}
## delta is the difference between the conditions
## sigma is the variance? i think? look at podcast around 2:25pm
```

What happens if you vary the random seed?

```{r}
## you will get slightly different data :>
## it should pawbly behave mostly the same though
## it just varies the data in the tables it generates
```
...
```{r}
## sim is the "simulation". so, which run of the experiment it's from, i think?
```
...
```{r}
## it now looks like it's pretty centered on zero, normally distributed.
## this is what they mean by empirically null - things were scrampled as if there is no effect.
## so the null - where there is no effect - is represented by the data having been scrampled over and over like that.
## look below at the color plots for a visualization & a better representation.
```
...
```{r}
## deltahat there is the... something.
## podcast at 2:45.
```
...
```{r}
## se stands for standard error btw.
```
